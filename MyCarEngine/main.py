...
class MyCar(Object):
    ...
    def get_acceleration(self, obj: any, dt: float, a_max: float = 10, da_max: float = 1e10, noise: float = 0.01):
        ...
        if self.flag < 5:
            ...
            if self.k is None:  # Определение параметров на первом шаге
                self.k = clip(v_diff**2 / x_diff**2, 0, 300)
                print(f"{self.name}: k={self.k}< {a_1}, {v_1}")
            k_x = self.k
    ...
    def __init__(self, dt: float = 1., vy: float = 10, frames: int = 10, saving: bool = False, many_cars: bool = False):
        ...
        if many_cars:
            self.many_cars = [MyCar(self.d.images['cars'][2], self.d.images['icons'][0], [256, 256 + 50],
                              x_m=0, y_m=-9 - 5*i, front_and_back=[1.8, -1.8], vy=self.vy + np.random.uniform(0.01),
...
if __name__ == '__main__':
    p = Process(frames=1000, dt=0.025, vy=10, saving=False, many_cars=True)
    p.process()
    p.plot_process()
    # p.plot_acceleration()
